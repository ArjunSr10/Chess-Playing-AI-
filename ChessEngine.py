...
class GameState:
    ...
    def getPawnMoves(self, row, col, moves):
        ...
        """
        piece_pinned = False # check the pin piece and the directions
        pin_direction = ()
        for i in range(len(self.pins) - 1, -1, -1): # going through the list of pin pieces and seeing if the piece the pawn that im currently looking at is a pin piece
            if self.pins[i][0] == row and self.pins[i][1] == col:
                piece_pinned = True # if so set this to true
                pin_direction = (self.pins[i][2], self.pins[i][3]) # save the direction at which its being pinned from
                self.pins.remove(self.pins[i]) # remove from list pins
                break # the idea here is im just getting info about the pin itself
    
        if self.white_to_move:
            move_amount = -1
            start_row = 6
            enemy_color = "b"
            king_row, king_col = self.white_king_location # get the row and column of white king
        else:
            move_amount = 1
            start_row = 1
            enemy_color = "w"
            king_row, king_col = self.black_king_location # get the tuple of the black king location
    
        if self.board[row + move_amount][col] == "--":  # 1 square pawn advance
            if not piece_pinned or pin_direction == (move_amount, 0): # before we append the 1 square we have to make sure that the piece is not pinned or the pin direction is in the direction that the piece wants to move
        ...
        if col - 1 >= 0:  # capture to the left
            if not piece_pinned or pin_direction == (move_amount, -1):
                ...
                if (row + move_amount, col - 1) == self.enpassant_possible: # we've already established enpassant is possible, before we add this move in here we're going to have to determine whether or not there is an attacking piece attacking our king and whether or not there is a blocking piece in front
                    attacking_piece = blocking_piece = False # two boolean variables both start to be false, these variables if they get set to true then i know that there is an attacking/blocking piece in the way
                    if king_row == row: #we need to know where the king is, split into two cases where the pawns are to the left or the right of the king
                        if king_col < col:  # king is left of the pawn
                            # inside: between king and the pawn;
                            # outside: between pawn and border;
                            inside_range = range(king_col + 1, col - 1) # define two ranges, 1 to the left of our pawn, we know that the enemy's pawn is going to be in the column minus 1 square
                            outside_range = range(col + 1, 8) # outside range is going to be in the opposite side of the second pawn
    ...
    def getRookMoves(self, row, col, moves):
        ...
        for i in range(len(self.pins) - 1, -1, -1):
            if self.pins[i][0] == row and self.pins[i][1] == col:
                ...
                if self.board[row][col][ # if its a queen you dont want to remove it from the list when you go through our and so our queen moves generate rook
                    1] != "Q":  # can't remove queen from pin on rook moves, only remove it on bishop moves
                    self.pins.remove(self.pins[i])
                break
    
        directions = ((-1, 0), (0, -1), (1, 0), (0, 1))  # up, left, down, right. Defined what these directions are going to be (pairs of coords) using the first variable to represent how im changing the row and the second variable to represent how im changing the column, a list of tuples. First tuple is the direction that is moving the row minus 1 and the column 0 so this is the direction up, second one is left and so on. these are the directions a rook can move
        enemy_color = "b" if self.white_to_move else "w" # determines the enemy_color depending on who's turn it is
        for direction in directions: # go through the directions variable
            for i in range(1, 8): # going through potentially 7 values
                end_row = row + direction[0] * i # take the end row where my rook could potentially move and  set it equal to its starting position plus whatever direction its moving in timsed by i where i is 1, then 2 and so on. So potentially moving up to 7 rows.
                end_col = col + direction[1] * i # same thing with column
                if 0 <= end_row <= 7 and 0 <= end_col <= 7:  # check for possible moves only in boundaries of the board
                    if not piece_pinned or pin_direction == direction or pin_direction == (
                            -direction[0], -direction[1]): # if piece is not pinned or the pin direction is in the direction or the opposite direction
                        end_piece = self.board[end_row][end_col]
                        if end_piece == "--":  # empty space is valid, check to make sure empty square
                            moves.append(Move((row, col), (end_row, end_col), self.board)) # if empty append move
                        elif end_piece[0] == enemy_color:  # capture enemy piece, if not empty
                            moves.append(Move((row, col), (end_row, end_col), self.board)) # valid move
                            break # cant jump enemy piece so for inner for loop breaks and another direction is checked
                        else:  # friendly piece then cant capture so the break
                            break
                else:  # off board
                    break
    ...
    def getKnightMoves(self, row, col, moves):
        ...
        knight_moves = ((-2, -1), (-2, 1), (-1, 2), (1, 2), (2, -1), (2, 1), (-1, -2),
                        (1, -2))  # up/left up/right right/up right/down down/left down/right left/up left/down. Knights can move in eight potential squares (L shape movement)
        ally_color = "w" if self.white_to_move else "b" # this time check allyColor
        for move in knight_moves:
            end_row = row + move[0] # dont have to times by i as we're adding set values to row and column
            end_col = col + move[1]
            if 0 <= end_row <= 7 and 0 <= end_col <= 7: # check to see if on board
                if not piece_pinned:
                    end_piece = self.board[end_row][end_col] # grab piece
                    if end_piece[0] != ally_color:  # so its either enemy piece or empty square
                        moves.append(Move((row, col), (end_row, end_col), self.board)) # knight can hop pieces so dont have to worry about running into ally piece as long as endpiece has an enemy piece or end square then this is a valid move
    ...
    def getBishopMoves(self, row, col, moves):
        ...
        directions = ((-1, -1), (-1, 1), (1, 1), (1, -1))  # diagonals: up/left up/right down/right down/left, change row and column by same amount to move diagonally e.g. both by negative 1 and negative 1
        enemy_color = "b" if self.white_to_move else " w"
        # same logic as rook moves
    ...
    def getQueenMoves(self, row, col, moves):
        ...
        """
        self.getBishopMoves(row, col, moves) # a queen is just a rook and a bishop, call other two functions to get rook and bishop moves
